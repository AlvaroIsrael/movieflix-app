...
export const AuthProvider: React.FC = ({ children }: PropsWithChildren<ReactNode>) => {
  ...
  const signIn = useCallback(async ({ email, password }: SignInCredentials) => {
    const response = await api.post<AuthState>('/v1/sessions', { email, password });
    const { token, user } = response.data;

    localStorage.setItem('@MovieFlix:token', token);
    localStorage.setItem('@MovieFlix:user', JSON.stringify(user));
...
export const useAuth = (): AuthContextState => {
  ...
  if (!context) {
    throw new Error('useAuth methods must be used within an AuthProvider.');
  }
